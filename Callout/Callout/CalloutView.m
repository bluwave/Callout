...
- (CGFloat)relativeParentXPosition
{
    if (self.superview)
    {
        CGPoint parentOrigin = [self.superview convertPoint:self.frame.origin fromView:self.superview];
...
//    [super drawRect:rect];
    ...
    /* figure out what to do w/ these variables */
    float yShadowOffset = 6;
    ...
    NSLog(@"[%@].%@ parentX: %f", [self class],NSStringFromSelector(_cmd), parentX);

    //Determine Size
    rect = self.bounds;
    rect.size.width -= stroke + 14;
    rect.size.height -= stroke + CalloutMapAnnotationViewHeightAboveParent - offsetFromParent.y + CalloutMapAnnotationViewBottomShadowBufferSize;
...
-(void)showFromAnchorPoint:(CGRect)anchorPoint inView:(UIView*)view animated:(BOOL)animated
{
    CGSize size = self.bounds.size;
//    CGRect frame = CGRectMake(fabsf( anchorPoint.origin.x - floorf(size.width / 2) ), anchorPoint.origin.y - size.height, size.width, size.height);
    CGFloat anchorHalf = anchorPoint.size.width / 2;
    CGFloat boundsHalf = size.width / 2;
    CGFloat x = anchorHalf - boundsHalf;
    CGRect frame = CGRectMake( x, anchorPoint.origin.y - size.height, size.width, size.height);

//    NSLog(@"[%@].%@ anchorPt: x: %f y: %f w: %f h: %f", [self class], NSStringFromSelector(_cmd), anchorPoint.origin.x, anchorPoint.origin.y, anchorPoint.size.width, anchorPoint.size.height);
//    NSLog(@"[%@].%@  w: %f h: %f", [self class], NSStringFromSelector(_cmd), size.width, size.height);
//    NSLog(@"[%@].%@ frame: x: %f y: %f w: %f h: %f", [self class], NSStringFromSelector(_cmd), frame.origin.x, frame.origin.y, frame.size.width, frame.size.height);
    self.frame = frame;
